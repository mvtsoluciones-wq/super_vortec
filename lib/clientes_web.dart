...
class _ClientesWebModuleState extends State<ClientesWebModule> {
  ...
  // Lista simulada de clientes (Base de datos local por ahora)
            ...
            child: Container(
              ...
              child: Form(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("NUEVO CLIENTE", style: TextStyle(color: brandRed, fontWeight: FontWeight.w900, fontSize: 16)),
  ...
  Widget _buildClientesTable() {
    return Container(
      ...
      child: DataTable(
        ...
        columns: const [
          DataColumn(label: Text("CLIENTE", style: TextStyle(color: Colors.white70, fontSize: 10))),
          DataColumn(label: Text("IDENTIFICACIÓN", style: TextStyle(color: Colors.white70, fontSize: 10))),
          DataColumn(label: Text("VEHÍCULO", style: TextStyle(color: Colors.white70, fontSize: 10))),
          DataColumn(label: Text("ACCIONES", style: TextStyle(color: Colors.white70, fontSize: 10))),
        ],
        rows: _clientesRegistrados.map((cliente) {
          return DataRow(cells: [
            ...
            DataCell(Text(cliente['id']!, style: const TextStyle(color: Colors.white54, fontSize: 12))),
            DataCell(Text(cliente['vehiculo']!, style: const TextStyle(color: Colors.white54, fontSize: 12))),
            DataCell(Row(
              children: [
                IconButton(icon: const Icon(Icons.edit, size: 18, color: Colors.blue), onPressed: () {}),
                IconButton(icon: const Icon(Icons.history, size: 18, color: Colors.green), onPressed: () {}),
              ],
            )),
          ]);
        }).toList(),
